...
protected:
    ...
    /**
     * RandomIt first, RandomIt last: 迭代器范围，即排序后的键序列的起始和结束位置。
     * size_t epsilon, size_t epsilon_recursive: 控制 PGM 索引的构建。epsilon 用于控制搜索范围的大小，
     * std::vector<Segment> &segments: 存储构建出的 PGM 索引的线段。
     * std::vector<size_t> &levels_offsets: 存储每个层级的起始位置在 segments 中的偏移量。
    */
    ...
    static void build(RandomIt first, RandomIt last,
        ...
        segments.reserve(n / (epsilon * epsilon));  // 为什么不是 n/(2*epsilon)
